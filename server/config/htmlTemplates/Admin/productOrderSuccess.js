...
exports.adminNewProductOrderHtml = (order) => {
...
<head>
    ...
    <title>New Product Order Notification - The Link Hangouts</title>
    <style type="text/css">
...
<body style="margin: 0; padding: 0; min-width: 100%; background-color: #f4f4f4;">
    ...
    <span class="preheader" style="color: transparent; display: none !important; height: 0; max-height: 0; max-width: 0; opacity: 0; overflow: hidden; mso-hide: all; visibility: hidden; width: 0;">
        New Product Order Received! Order #${order._id} from ${order.name}.
    </span>
                    ...
                    <tr>
                        <td class="padding-mobile" style="padding: 30px 40px;">
                            ...
                            <p style="font-family: Arial, Helvetica, sans-serif; font-size: 16px; line-height: 1.6; color: #333333; margin: 0 0 20px 0;">
                                A new product order has been placed on The Link Hangouts. Please review the details below:
                            </p>
                            ...
                            ${order.cart.items.filter(item => item.type === 'product').map(product => `
                                <p style="font-family: Arial, Helvetica, sans-serif; font-size: 16px; line-height: 1.6; color: #333333; margin: 0 0 5px 0;">
                                ...
                                <p style="font-family: Arial, Helvetica, sans-serif; font-size: 16px; line-height: 1.6; color: #333333; margin: 0 0 5px 0;">
                                    <strong>Delivery:</strong> ${product.needsDelivery ? 'Yes - Delivery Required' : 'No - Pickup at Event'}
                                </p>
                                ${product.needsDelivery && product.deliveryInfo ? `
                                    <p style="font-family: Arial, Helvetica, sans-serif; font-size: 16px; line-height: 1.6; color: #333333; margin: 0 0 5px 0;">
                                        <strong>Delivery Address:</strong><br>
                                        ${product.deliveryInfo.name}<br>
                                        ${product.deliveryInfo.address.street}<br>
                                        ${product.deliveryInfo.address.city}, ${product.deliveryInfo.address.state}<br>
                                        Phone: ${product.deliveryInfo.phoneNumber}
                                    </p>
                                ` : ''}
