...
exports.productOrderEmailHtml = (order) => {
    ...
    let productNamesFormatted;
    if (uniqueProductNames.length > 1) {
        const lastProduct = uniqueProductNames[uniqueProductNames.length - 1];
        const otherProducts = uniqueProductNames.slice(0, -1);
        productNamesFormatted = otherProducts.join(', ') + ' and ' + lastProduct;
    } else {
        productNamesFormatted = uniqueProductNames[0];
    }
...
<head>
    ...
    <title>Your Link Hangouts Product Order Confirmation!</title>
    <style type="text/css">
...
<body style="margin: 0; padding: 0; min-width: 100%; background-color: #f4f4f4;">
    ...
    <span class="preheader" style="color: transparent; display: none !important; height: 0; max-height: 0; max-width: 0; opacity: 0; overflow: hidden; mso-hide: all; visibility: hidden; width: 0;">
        Your order for ${productNamesFormatted} has been confirmed! Details inside.
    </span>
